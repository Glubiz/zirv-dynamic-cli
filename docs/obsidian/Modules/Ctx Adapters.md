...
`adapters::all(bin)` returns `vec![ClaudeAdapter, CodexAdapter]` — the full registry. `adapters::select(name, command, cfg: &CtxConfig)` picks one adapter, in priority order: an explicit `--agent` name, else `detect()` against the wrapped command's argv, else default to claude (`cfg.agent_bin` supplies the adapter's `bin` override). In every arm, before the chosen adapter's own `ready()` is called, `select` checks `cfg.agents.refusal(adapter.name())` (the `.settings.toml` gate — see below) and returns that refusal as an `Err` if the agent is disabled. An unready-but-enabled adapter (currently: codex, always) is still rejected at selection time by `ready()`, just after the gate check rather than before.
...
What *is* implemented, per command comments citing `docs/superpowers/notes/2026-07-31-codex-cli-facts.md` and `docs/superpowers/notes/2026-08-01-system-prompt-injection-facts.md` (verified against `codex --help`/`codex exec --help`): `headless_cmd` builds `codex exec <prompt> <extra...>` (codex has no `--session-id` flag — it always mints its own id), `interactive_cmd` builds `codex [<prompt>] <extra...>`, `distiller_cmd` builds `codex exec --model <model>` (codex reads its prompt from stdin, so it's built the same way as claude's), `model_args` emits `-m/--model <model>` (verified present on top-level `codex --help` too, so the dashboard's orchestrator pane can select a model on codex the same way it does on claude), and `transcript_path` scans `~/.codex/sessions/<YYYY>/<MM>/<DD>/...` recursively by filename suffix, since codex nests transcripts under a date directory that a bare session id can't predict. No `session_pin_args` override either, for the same reason and one stronger: codex has no `--session-id` flag at all, so an interactive pane cannot be pinned and the trait default (empty) stands. No `resume_args` override: codex's own resume story (a `--last`/session-id flag for the interactive launch) is unverified against the real CLI, so the trait default (`None`) stands — `dash::roster::restore_argv` falls back to a plain prompt-carrying relaunch for this adapter. This matches the repo's CLAUDE.md and the ctx CLI's own `about` text: codex is tracked but not ready.

**Latent gap worth knowing before codex is completed:** `CodexAdapter::base()` does not route through `resolve_program` and overrides neither `launches_through_cmd_shim` nor `system_prompt_file_flag` — so it has none of the Windows `cmd.exe`-shim defenses claude's own `base()` applies (see "Windows launcher rewriting" above and [[Known Issues]]). Inert today only because `ready()` always errors, so no codex launch ever reaches a spawn. Completing codex support must mirror claude's `resolve_program` + shim detection + forced system-prompt-file delivery, or it reintroduces the same Windows `.cmd` reparse RCE class the round-9 fix (2026-08-14) just closed for claude.
