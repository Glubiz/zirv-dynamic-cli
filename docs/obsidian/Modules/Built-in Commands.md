...
- **Gotchas:** `zirv ctx …` and top-level `zirv --help`/`-h` are both matched against raw `argv` *before* clap parses anything — a `.zirv/ctx.yaml` script named `ctx` is permanently shadowed, and clap's own auto-generated help for the `Input` struct never fires.

## Purpose

`main.rs` is the CLI's dispatch table: a fixed set of built-ins (`help`, `version`, `init`, `create`, `ctx`) checked before anything falls through to "treat this word as a script name in `.zirv/`".
...
2. **Raw-argv top-level help check** (`is_top_level_help`) — `argv[1]` is exactly `--help` or `-h` (not a script's own `--help` parameter, which lands later). Bypasses clap so `zirv help`'s rich script listing runs instead of clap's generated help for `Input`.
3. **`Input::parse()`** — clap parses the rest.
4. **`misplaced_create_flag` guard** — `--name`/`--shortcut`/`--global` live on the shared `Input` struct so clap accepts them for *every* command; used outside `create`/`c` they are refused with a named error instead of being silently swallowed as (and eating) the next positional argument.
5. **Built-in match** on `input.command`: `help|h`, `version|v`, `init|i`, `create|c` each dispatch to their module and return.
6. **Fallback**: resolve `input.command` as a script via `Input::get_file_path`, parse it with `utils::file_to_script`, and run it through `script_runner::execute`.
