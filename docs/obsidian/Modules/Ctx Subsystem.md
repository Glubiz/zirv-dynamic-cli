...
- **`agent`** — one-shot delegation of a task to a supervised headless worker on another enabled harness, driven through `exec` (also `zirv agent <name> <prompt>`). When this process is itself running inside a dashboard pane (`spawnreq::DASH_REQUESTS_ENV` set and still live), it asks the dashboard to spawn the delegated agent as a fresh pane instead of running headless in its own subshell — see [[Ctx Supervisors]].
