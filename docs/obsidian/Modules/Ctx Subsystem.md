...
- **`chat`** — start an interactive orchestrator session on the resolved adapter, driven through `wrap` (also reachable as the top-level alias `zirv chat`, or bare `zirv` in a repo with a local `.zirv` directory and both stdin and stdout attached to a real terminal — see [[Built-in Commands]])
...
- **`nudge`** — wake a live session early with a message, resolved against the live session registry by short-id prefix
