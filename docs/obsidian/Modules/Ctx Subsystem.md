...
- **`chat`** — start an interactive orchestrator session on the resolved adapter, driven through `wrap` (also reachable as the top-level alias `zirv chat`, or bare `zirv` in a repo with a `.zirv` directory and a real terminal — see [[Built-in Commands]])
