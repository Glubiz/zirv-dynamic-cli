...
| Ctx adapters | `src/commands/ctx/adapters/{mod,claude,codex}.rs`, `src/commands/ctx/{catalogue,transcript_source}.rs` | [[Ctx Adapters]] | `AgentAdapter` trait; claude (full capabilities) and codex (launch-supported; turn-boundary/token event parsing and rot scoring now derived from its rollout JSON, issue #86; tool-call/tool-result/compaction mapping and turn signal remain issue #11); `catalogue.rs` (issue #381) is the pure per-vendor model ladder/price/window table both adapters delegate to; `transcript_source.rs` (issue #382) materializes a JSON-snapshot/SQLite harness's transcript into a shadow JSONL, dormant until a wave-1 adapter (issues #384-#386) exists |
