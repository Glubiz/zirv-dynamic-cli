...
| Built-ins & entry point | `src/main.rs`, `src/input.rs`, `src/commands/{mod,create,init,help,version,setup}.rs` | [[Built-in Commands]] | Dispatch table; `help`/`version`/`init`/`create` and their aliases; `setup` (guided onboarding/`reset`/`restore`, one manifest-tracked backup mechanism with count-capped retention, and the once-only memory-harvest/statusLine-wrap offers) |
...
| Ctx supervisors | `src/commands/ctx/{run_loop,exec,wrap}.rs` + `{signal,supervise,term}.rs` + `dash/{mod,pane,ui,spawnreq,roster,hit,actions,notify}.rs` + `{seat,rollover}.rs` | [[Ctx Supervisors]] | The three process supervisors, turn-signal sockets, shared process/terminal primitives, the `dash` session multiplexer `zirv chat` opens on a capable terminal (pure mouse hit-testing, the one action-descriptor table behind help/menu/header/palette, and the pure attention-notice reducer — issue #354), and (issue #358) the persisted logical orchestrator seat plus the automatic cross-harness rollover transaction built on the #84 handover seams |
| Ctx adapters | `src/commands/ctx/adapters/{mod,claude,codex,copilot,droid,gemini,opencode,pi,qwen}.rs`, `src/commands/ctx/{catalogue,transcript_source}.rs` | [[Ctx Adapters]] | `AgentAdapter` trait; eight registered adapters (claude, codex, copilot, droid, gemini, opencode, pi, qwen). claude has full capabilities; codex is launch-supported (turn-boundary/token event parsing and rot scoring derived from its rollout JSON, issue #86; tool-call/tool-result/compaction mapping and turn signal remain issue #11); the six wave-1/2 adapters (issues #384-#389, umbrella #396, 2026-09-07) are verified from source/docs/a live binary rather than probed against an installed CLI — see [[Ctx Adapters]]'s "wave-1/2 harness adapters" section for the per-adapter capability gaps. `catalogue.rs` (issue #381) is the pure per-vendor model ladder/price/window table every adapter delegates to; `transcript_source.rs` (issue #382) materializes a JSON-snapshot/SQLite harness's transcript into a shadow JSONL — `opencode.rs` is its first real caller (SQLite) |
...
| `hook` | Agent hook entrypoints (`Stop`, `Prompt`, `PreCompact`, `Notify`, `SessionStart` — issue #244, re-injects the latest handoff on resume/clear); must never exit non-zero on `Stop`. |
...
| Command safety policy, `[safety]`, the wired `PreToolUse` hook | [[Command Safety]], [[Ctx Adapters]] |
...
- **codex is launch-supported, not feature-parity**: `CodexAdapter::ready()` mirrors claude's (only an unresolvable binary fails it), so `--agent codex` selects and launches. `capabilities().events`/`system_prompt` are now `true` (turn-boundary/token events derive from the rollout JSON, issue #86; developer-instructions injection on direct launches) — `marker_signal`/`token_usage`/`turn_signal` stay `false` (usage-window data comes from a separate out-of-band collector, see [[Usage and Pacing]]). Don't assume adapter parity between claude and codex when reading `adapters/codex.rs`; tool-call/tool-result/compaction event mapping and the turn-signal socket mechanism remain issue #11. See [[Ctx Adapters]], [[Known Issues]].
...
`StateDir::resolve` roots at `ZIRV_CTX_STATE_DIR`, else the OS state dir, else the OS local-data dir, then `zirv/ctx`. Subpaths: `handoffs/<repo_slug>/` (stored handoff docs), `s/` (turn-signal sockets, short name for the unix path-length limit), `logs/decisions.jsonl` (append-only context decision log), `logs/safety-decisions/<UTC-day>.jsonl` (privacy-preserving command verdicts with command/policy SHA-256 identities, never raw command text; retained 30 days as of 2026-09-06, `append_safety` prunes older buckets on every append -- see [[Known Issues]]'s `prune_safety_buckets` perf-note entry), `usage.json` (single machine-wide file, merged across sessions), `scoring/` (per-transcript incremental-scoring checkpoints), `status-snapshots/` (issue #246  --  one `StatusSnapshot` JSON per session, keyed by `input_hash(session)`, holding `zirv ctx status --diff`'s previous rendered sections), `objective/<repo_slug>.json` (issue #285  --  one durable objective record per repo, written with no interprocess lock -- see [[Known Issues]]), `rollouts/<short>.path`/`<short>.floor` (2026-09-06 -- the codex transcript pin `adapters/codex.rs` resolves once per session and never prunes, see [[Known Issues]]), `attention/<short>.json`/`<short>.lock` (issue #349's composed session status, the `.lock` sibling added 2026-09-06 to serialize the ledger's own read-modify-write), `shadow/<short>.jsonl` (issue #382, v3.31.0 -- a materialized shadow transcript for a harness whose native transcript is a JSON snapshot or SQLite database, pruned by mtime past 30 days idle, dormant until a wave-1 adapter calls it -- see [[Ctx Adapters]]), `ledger.sqlite` (issue #422, v3.36.0 -- one WAL-mode sqlite file, indexed `compactions` table, one row per `PostToolUse` compaction decision `hook::run_posttool` recorded, retained 90 days, pruned on roughly one write in 64; a separate file from the shadow-transcript database above so compaction bookkeeping never contends with a harness's own live-file lock -- see [[Ctx Subsystem]]'s `savings` verb entry). Unix directories are `0700` and files `0600`; Windows has no equivalent and is a no-op there. See [[Ctx Subsystem]].
