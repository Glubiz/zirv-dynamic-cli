...
### `feat/chat-token-economy` — worker model defaults, PreToolUse seat guard, token-economy prompts, capacity marker (2026-08-18, v2.12.0)
**Status (2026-08-18):** Four commits (`059ad3c`, `fd5a709`, `b2f457d`, `1e1ca45`) on `feat/chat-token-economy`, committed. New `[worker]` config defaults a delegated headless worker to claude's own `"sonnet"` (codex: unmanaged) instead of the operator's interactive seat model. New `zirv ctx hook pretool` (PreToolUse) guard deterministically denies a subagent dispatch that would fork or silently inherit an expensive orchestrator seat, fixed same-day to follow the *effective* launch model and to gate strictly on a real dispatch's own non-empty prompt. `ORCHESTRATOR_PROMPT`/`HARNESS_PROMPT` rewritten (v5) for mandatory explicit model tiers, a fork ban, token-economy briefs, and single-reviewer-effort `/code-review`. New per-agent `capacity = "small"` marker in `.zirv/.settings.toml`, rendered in the harness roster. `Cargo.toml` bumped to 2.12.0. See [[Work Journal]] and [[Decision Log]] (two 2026-08-18 entries).
**Next:** no PR opened yet for this branch. Two follow-ups live outside this repo, not yet done: the operator must wire `zirv ctx hook pretool` manually into `~/.claude/settings.json` (matcher `Agent|Task`) for the guard to run at all, and a codex `capacity = "small"` marker (if wanted) needs setting in the operator's home `.zirv/.settings.toml`. See [[Known Issues]] for the unrelated `~/.codex/config.toml` model-pin 400 gotcha recorded during this round.
