...
## The dashboard's chrome is clickable now, but four of issue #354's five phases are still missing

Recorded 2026-09-05 (`feat/354-dash-ux-audit`, issue #354 phase 1). Pointer events used to reach only the focused pane's grid: a click on the sidebar was dropped on the floor, so the roster was keyboard-only while looking exactly like a list you could click. That is closed — `dash::hit::hit_test` plus `dash::route_mouse` give rows, group headers, the summary line and the header hints real pointer targets, and the sidebar wheel scrolls the roster (see [[Ctx Supervisors]]'s "The 44-column sidebar, the group tree, disclosure and pointer ownership"). What is deliberately **not** in yet, and will look like a half-wired feature until it is:

- **Attention is still only working/idle/dead** (phase 2). The `▲` needs-action, `✗` failed and `◆` done-unread glyphs the design calls for, their rollups in the summary line and group headers, and the done-unread acknowledgement all wait on `attention::SessionStatus` being cached on the `FactsCache` cadence. `RowState` is what the glyph column and `ui::rollup` can express today, and neither invents the rest. The `reason` disclosure line is therefore always `none · <state>`.
- **Right-click and `^A c` raise a notice, not a menu** (phase 3). `DashAction::ContextMenu(row)`/`ContextActions` capture the target and push `actions menu arrives in phase 3`; the menu, `^A i inspect`, a real Restore action and the scrollable list dialogs are phase 3's work. The header cluster deliberately does not advertise `^A i`/`^A r` until they exist.
