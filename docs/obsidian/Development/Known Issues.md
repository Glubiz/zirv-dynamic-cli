...
## A forged dashboard spawn request can point a fresh worker pane's writes at any git repo the dashboard user can reach

**Recorded 2026-08-31 (`fix/bug-batch-227-228-229-232-233`, issue #228, v3.1.0), widens the accepted #179 trust boundary.** `--workdir`/`--headless` on `zirv agent`/`zirv ctx agent` let a pane-spawn request carry a new `SpawnRequest.workdir` field, re-validated server-side by `dash::resolved_spawn_cwd` the same way `SpawnRequest.cwd` already is (canonicalised, must exist, must be a directory, must be inside a git repository) — but "inside *a* git repository" is a much wider gate than "inside *this dashboard's own* repo family," which is what `accepted_spawn_cwd` enforces for `cwd`. A same-uid sibling process able to write into a live dashboard's requests directory at all — the same filesystem intake boundary issue #179 already accepted as a trust boundary, still unclosed (see `dash/mod.rs`'s own doc comment) — can therefore now aim a forged request's `workdir` at any git checkout the dashboard's own user account can reach, not only a repo related to the dashboard's own launch. Not fixed here: closing it needs the same unix-socket `SO_PEERCRED`/pid-ancestry attribution issue #179 was filed for, applied to `workdir` as well as the request's own identity. See [[Ctx Supervisors]]'s dashboard section and [[Untrusted Configuration]] for the re-validation mechanics.
