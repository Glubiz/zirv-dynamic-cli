...
## The dashboard's chrome is clickable and attention-aware now, but three of issue #354's five phases are still missing

Recorded 2026-09-05 (`feat/354-dash-ux-audit`, issue #354 phases 1 and 2). Pointer events used to reach only the focused pane's grid: a click on the sidebar was dropped on the floor, so the roster was keyboard-only while looking exactly like a list you could click. That is closed — `dash::hit::hit_test` plus `dash::route_mouse` give rows, group headers, the summary line and the header hints real pointer targets, and the sidebar wheel scrolls the roster. **Phase 2 also closed the attention gaps**: the glyph column, the summary/group rollups and the `reason`/`since` disclosure lines are projections of the cached `attention::SessionStatus` (`ui::glyph_for`, `DiskFacts::attention`, loaded on the `FactsCache` cadence, never per frame), `◆` clears only after an unoccluded render of the focused pane rather than on any focus change, completed workers keep a row (capped at 8), and `^A Left`/`^A Right` share one fold reducer with the header click. See [[Ctx Supervisors]]'s "The 44-column sidebar…" and "Attention glyphs, done-unread acknowledgement and retained ended rows" sections. What is deliberately **not** in yet, and will look like a half-wired feature until it is:

- **`^A i inspect` opens the errors/evidence overlay, not a real inspector** (phase 3). The header offers the chord for a needs-action or ended row, and the keyboard binds it, but both land on `DashAction::ShowErrors` — the closest thing that exists to "show me what happened here" — until phase 3 builds the per-session inspector with its own evidence view. Drawing the hint and having it do nothing would be worse; renaming the action when the inspector lands is the follow-up.
- **A retained ended row cannot be dismissed by hand** (phase 3). It leaves the roster only when `MAX_RETAINED_ENDED_ROWS` (8) pushes it out or the dashboard exits; the design's "operator dismisses it" is the close/stop action the phase 3 context menu introduces, and `Enter`/click behaviour is deliberately unchanged. Related: a clean-exit `◆` on a retained row has no acknowledgement path, because acknowledgement requires the pane to be *focused* and a retained row is selectable-only — it clears only if the operator had already seen that revision while the pane was alive.
- **Right-click and `^A c` raise a notice, not a menu** (phase 3). `DashAction::ContextMenu(row)`/`ContextActions` capture the target and push `actions menu arrives in phase 3`; the menu, a real Restore action and the scrollable list dialogs are phase 3's work. The header cluster deliberately still does not advertise `^A r`.
- **An open overlay swallows every pointer event, including the wheel** (phase 3 gives overlays their own hit regions). This is correct-but-blunt: a dialog cannot be scrolled or clicked, only driven from the keyboard.
- **`branch` in the disclosure is always the placeholder.** Filling it needs a *cached* branch; the render path must never shell out to git, and no cheap cached value exists yet.
