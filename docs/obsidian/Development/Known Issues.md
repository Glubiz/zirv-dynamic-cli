...
- **The first-run tip's flag is written at launch, not at dismissal.** A session that opens the dashboard and quits without pressing anything has still "seen" it. Writing it on dismissal instead would re-show the tip after every crash or fast quit, which is worse; the flag file (`<state>/dash/tip-seen`) can simply be deleted to get the tip back.
