...
<!-- Updated 2026-09-09 (release/3.41.0-filters-and-endpoints, issues #417/#395): recorded that `[[output.filter]]`'s `catch_unwind` fallback is a dev/test-only net given `panic = "abort"`, and that `provider_for_agent_and_model`/`provider_for_agent_name` stay endpoint-unaware by design -->
...
<!-- Updated 2026-09-06 (feat/inner-workings-audit, T9 fix track, zirv ctx agent --headless removal): recorded that a dashboard pane still cannot enforce --max-tool-calls (no verified tool-call counter), the one ceiling of the three that travelled onto SpawnRequest but stayed unenforced -- --max-restarts/--timeout-secs both got real pane-side mechanisms in the same track -->
...
<!-- Updated 2026-09-05 (feat/354-dash-ux-audit, issue #354 phase 5): attention-transition notices, error acknowledgement with repeat counts, the dashboard-level inspector and the dismissal-timed tip flag are in -- all five phases closed; rewrote this entry to list the residuals that remain after them -->
<!-- Updated 2026-09-05 (feat/354-dash-ux-audit, issue #354 phase 4): the one action-descriptor table, the `^A p` palette, the searchable help built on it, the one Esc/Enter rule and the persisted first-run tip are in; the only remaining gap is phase 5 notifications and error acknowledgement -->
<!-- Updated 2026-09-05 (feat/354-dash-ux-audit, issue #354 phase 3): the shared scrollable list viewport, overlay pointer targets, the context menu with disabled-with-reason entries, the per-session inspector, real `^A r` restore and retained-row dismissal are in; the remaining gaps are the phase 4 palette/help and phase 5 notifications -->
<!-- Updated 2026-09-05 (feat/354-dash-ux-audit, issue #354 phase 2): attention glyphs/rollups, the render-gated done-unread acknowledgement, retained completed-worker rows and the shared collapse reducer are in; the remaining gaps are the phase 3 inspector, retained-row dismissal and the context menu -->
<!-- Updated 2026-09-05 (feat/354-dash-ux-audit, issue #354 phase 1): recorded what the clickable-chrome phase actually closes (sidebar clicks, group toggles, header hints, the sidebar wheel) and the phase 2-5 gaps it deliberately leaves open -->
...
<!-- Updated 2026-09-04 (release/3.20.0-harness-batch-6, issue #317): recorded that the supervisor loop (`exec`/`loop`) sends no per-tick heartbeat on a task-claiming worker's behalf, and that silent-zero-exit detection for a `--task`-claiming worker only fires when the delegation also declares `--result-schema` -->
...
## `[[output.filter]]`'s `catch_unwind` fallback is a dev/test-only net

Recorded 2026-09-09 (`release/3.41.0-filters-and-endpoints`, issue #417). `output.rs`'s `apply_operator_output_filter` wraps rule application in `std::panic::catch_unwind` and falls back to the unfiltered generic scan on `Err`, so a defect in one operator's regex/replace rule can never take the whole compaction pass down -- in a *dev/test* build. The release profile is `panic = "abort"` (`Cargo.toml`, load-bearing for [[Ctx Supervisors]]'s `wrap`-never-degrades contract too), so in the actual shipped binary a panicking filter rule aborts the hook process outright rather than unwinding into the `catch_unwind` boundary at all -- claude still keeps the raw, unfiltered tool result in that case (the hook process dying is itself a fail-open for claude's own `PostToolUse` contract), but the "falls back gracefully" story is only ever exercised in a dev/test build. Not a bug to fix -- `panic = "abort"` is an intentional, repo-wide choice -- but worth knowing before trusting a `catch_unwind`-based test to predict release behavior for this or any other hook-path panic.

## `provider_for_agent_and_model`/`provider_for_agent_name` stay endpoint-unaware by design

Recorded 2026-09-09 (`release/3.41.0-filters-and-endpoints`, issue #395). These two `cfg`-less helpers (`adapters::mod`) answer "which provider slug does this agent name/model report usage under" from the static registry alone -- `ctor(None).provider_for_model(model)`, never `apply_endpoint_override`, since that helper needs a `&CtxConfig` to look up `cfg.endpoint.<name>` and neither function takes one. Every capacity-view caller that resolves a provider WITHOUT a live, endpoint-attached adapter instance in hand -- `fallback.rs`'s snapshot/route helpers, `dash/{mod,pane}.rs`'s pool/reservation labeling, `status.rs`'s usage-windows line, `rollover.rs`'s destination provider -- therefore reports `"anthropic"`/`"openai"` for a harness the operator has retargeted at `[endpoint.claude]`/`[endpoint.codex]`, not the vendor slug a real launch (`adapters::select`, which DOES call `apply_endpoint_override`) would attribute spend to. This is deliberate, not an oversight: teaching these two helpers about `cfg.endpoint.*` would mean either threading a `&CtxConfig` through every one of their many call sites, or re-reading config from inside a pure-registry lookup -- both a bigger change than the capacity/labeling views these back actually need today. See [[Ctx Adapters]]'s "Endpoint overrides" section and [[Usage and Pacing]]'s "Spend-only providers" section for the paths that DO resolve endpoint-aware (`AgentAdapter::provider()`/`provider_for_model()` on an instance that went through `apply_endpoint_override`).
...
- `build.rs` (Windows-only, via the `winresource` crate) embeds a real `VERSIONINFO` resource -- `ProductName`, `FileDescription`, `CompanyName`, `LegalCopyright`, `OriginalFilename`, `InternalName`, `FileVersion`/`ProductVersion` from `CARGO_PKG_VERSION` -- so the shipped exe at least identifies itself the way legitimate Windows software does. No-op on every other target, and also when a Windows exe is cross-compiled from a non-Windows host (the `winresource` build-dependency is host-gated); the release exe is built natively on a Windows runner.
...
## None of the six wave-1/2 harness adapters has been smoke-tested against a live installed process

Recorded 2026-09-07 (`feat/adapters-waves-1-2`, issues #384-#389, umbrella #396). `gemini`, `opencode`, `pi`, `copilot`, and `qwen` are verified entirely from source (bundled JS, GitHub source pinned at an exact release tag) or published documentation -- none of the five CLIs is installed on the machine that built these adapters, and node here is v19, too old to run the gemini/qwen forks even if they were. `droid` is the one exception: closed-source with no bundled JS to read at all, so it was instead verified by running the real compiled `@factory/cli-win32-x64` 0.213.0 binary directly, including one live end-to-end agentic turn against a local BYOK mock model server (no real `FACTORY_API_KEY` exists here) -- see `tests/fixtures/droid/README.md` for the exact reproduction. Every fact not exercised live is cited to a specific file/symbol in the adapter's own module doc comment rather than guessed, but a genuinely live end-to-end run (real binary, real API key, a real zirv-supervised session) has not happened for any of the six. Re-verify each adapter's launch/transcript-path/read-only claims the first time an operator actually installs one of these harnesses and runs a real session through `zirv ctx wrap`/`exec`/`agent`.

## OpenCode's own repo config can widen a zirv-injected agent's system prompt or read-only permission

Recorded 2026-09-07 (`feat/adapters-waves-1-2`, issue #385). `OpenCodeAdapter` delivers its per-launch system-prompt override and read-only permission deny through a zirv-owned agent definition materialized to a file pointed at by `OPENCODE_CONFIG` -- the only delivery channel OpenCode exposes (there is no `--config <path>` CLI flag upstream). OpenCode's own config loader merges `global config -> OPENCODE_CONFIG flag -> project config`, in that order (`config.ts`, verified against the published v1.18.29 source), so a repository's own committed `opencode.json` loads AFTER zirv's layer and can redefine the same agent name -- overriding the injected system prompt or re-opening the read-only deny zirv just set for a distiller/reviewer/read-only worker launch. Every other untrusted-config surface on [[Untrusted Configuration]] is held to a narrow-only fold; this one can currently be widened by repo-owned config instead. Not fixed here -- would need either a second, later-loading override layer OpenCode doesn't currently expose, or a launch-time verification pass reading the merged config back and refusing to proceed if the agent's own prompt/permission no longer matches what zirv wrote. See [[Ctx Adapters]]'s opencode section.

## Copilot's verified transcript row shapes carry no assistant final-text event

Recorded 2026-09-07 (`feat/adapters-waves-1-2`, issue #387). Every fact about `events.jsonl`'s row shapes comes from two independent, mutually-corroborating third-party sources (a GitHub employee's own blog post, and the open-source `ccusage` project's Copilot parser) rather than official docs -- GitHub has never published this format (open upstream feature request `github/copilot-cli#3551`, still open). Both sources agree on `session.start`/`user.message`/`assistant.turn_start`/`tool.execution_start`/`tool.execution_complete`/`session.shutdown`, but neither documents an event carrying the assistant's own final response text anywhere in the stream. `CopilotAdapter::parse_events`/`structural_context` therefore cannot populate `AssistantFinal.text`/`assistant_texts` from anything currently verified -- rot scoring and structural context for a copilot session are honest about turn boundaries and tool activity but blind to what the assistant actually said. Would need either a newer copilot-cli version that adds a documented event for this, or a third independent source confirming a field neither `ccusage` nor the blog post names. See [[Ctx Adapters]]'s copilot section.

## Droid's real token usage lives in a sibling file the adapter's `transcript_usage` trait method cannot reach

Recorded 2026-09-07 (`feat/adapters-waves-1-2`, issue #388). A live end-to-end run against a local BYOK mock model server confirmed droid's `.jsonl` transcript carries no token-usage fields at all (`inputTokens`/`outputTokens`/cache classes) -- every usage class instead lives in a sibling `<session-id>.settings.json` file, rewritten whole each turn (`tokenUsage`/`inclusiveTokenUsage`, cumulative). `AgentAdapter::transcript_usage` only ever receives the `.jsonl` transcript's own CONTENT, never a path it could use to open that sibling file, so there is structurally no way to implement it without breaking the trait's line-local contract every other adapter's implementation honours. Left at the trait default (`None`), matching `capabilities().token_usage: false` honestly rather than guessing. Would need a trait change (a path parameter alongside the JSONL content) to close, which no other adapter needs today -- worth revisiting only if a second adapter surfaces the same shape. See [[Ctx Adapters]]'s droid section.

## The model catalogue's OpenAI rungs carry no verified context window; `normalize_id`/`vendor_of` still carry a stale `#[allow(dead_code)]`

Recorded 2026-09-07 (`feat/adapters-wave-1-catalogue`, issues #381/#382); updated 2026-09-07 (`feat/adapters-waves-1-2`, issues #384-#389). Two narrow gaps: (1) every rung on `catalogue::OPENAI_RUNGS` has `context_window: None`, and the vendor's own `default_context_window` is also `None` -- no static per-model capacity is verified for codex's lineup, matching `CodexAdapter::context_window_tokens`'s pre-catalogue answer exactly (codex's *live* per-session window still comes from `context_window_hint` reading the rollout JSON's own `info.model_context_window`, unaffected by this) -- still open, no wave-1/2 adapter touches codex's own ladder. (2) **Partially resolved:** `catalogue::vendor_of` now has real production callers -- `opencode`/`pi`/`droid`/`qwen`'s `provider_for_model` all resolve a per-launch vendor through it (issues #385/#386/#388/#389) -- but the `normalize_id`/`vendor_of` functions in `catalogue.rs` still carry their original `#[allow(dead_code)]` attributes from before those callers existed, now stale rather than accurate; a harmless lint-suppression leftover, not a behavior bug, but worth removing the next time `catalogue.rs` is touched.

## Codex's own hook-trust gate may fail `zirv`'s registered hooks in a way this repo cannot fix

Recorded 2026-09-06 (`feat/326-track-d`, issue #326 secondary finding). Reported symptom: `codex exec` printing `hook: UserPromptSubmit Failed` / `hook: Stop Failed` for the two `HARNESS_HOOKS` entries `zirv setup apply` registers into `~/.codex/hooks.json` (`zirv ctx hook prompt`, `zirv ctx hook stop`). Investigated by running both registered commands by hand with a sample Claude-shaped `HookPayload` JSON on stdin (both exit 0, with the expected `[zirv]` marker/`additionalContext` output) and by running a live `codex exec --sandbox read-only "..."` round-trip in this repo, which printed `hook: UserPromptSubmit Completed` / `hook: Stop Completed` for both -- not reproducible against this machine's current state. `codex --help` documents `--dangerously-bypass-hook-trust` ("Run enabled hooks without requiring persisted hook trust for this invocation"), and `~/.codex/config.toml` carries a `[hooks.state]` table with one `trusted_hash = "sha256:..."` entry per registered hook, keyed on `<hooks.json path>:<event>:0:0`. The most likely (unconfirmed -- the hashing input was not reverse-engineered) explanation: codex hashes each hook's persisted definition and refuses to run one whose live `hooks.json` content no longer matches its last-trusted hash until an operator re-approves it, which an unattended `codex exec` has no TTY to do -- so a hook can fail immediately after `zirv setup apply` rewrites `hooks.json` (even with byte-identical *logical* content, if formatting differs) until the operator runs `codex` interactively once, or passes `--dangerously-bypass-hook-trust`, to re-trust it. If this is right, the fix is entirely on codex's side (or an operator workflow note), not inside zirv: `ensure_harness_hook`/`install_codex_hooks` write standard JSON and have no way to see or influence codex's own trust store. Re-open and re-investigate the next time the failure is caught live (ideally capturing `~/.codex/hooks.json`'s exact bytes and `config.toml`'s `[hooks.state]` block at the moment of failure, before anything re-trusts it) rather than assuming this entry's guess is confirmed.
## Claude Code auto-loads `.zirv/context/claude.md` as a nested CLAUDE.md on a case-insensitive filesystem

Recorded 2026-09-06 (`feat/326-track-c`, orchestrator-side token trims). On a case-insensitive filesystem (macOS default, and Windows), Claude Code treats any file literally named `claude.md` -- regardless of which directory it lives in -- as a nested CLAUDE.md, and auto-loads it into context the first time a session reads ANY file in that directory. This happens entirely outside zirv's own canonical-context dedupe (`compile.rs`'s `native_file_already_carries_canonical`/`context_layer_dedupe_pointer`), which only knows about the repo-root `CLAUDE.md`/`AGENTS.md` pair. A session that reads a file under `.zirv/context/` (to inspect `common.md`, say) can therefore load `.zirv/context/claude.md` a second time, uncounted and with no dedupe pointer naming it. Workaround: avoid reading files under `.zirv/context/` from inside a session. zirv cannot fix this -- it has no way to suppress Claude Code's own nested-CLAUDE.md auto-load.

## `objective::store` has no interprocess lock

Recorded 2026-09-06 (`feat/inner-workings-audit-2`, second full-codebase audit). `objective.rs`'s `store` is a plain private-dir-then-atomic-write (matching `group::create`'s own shape), but unlike `group.rs` (which took a `GroupLock`) and `memory.rs` (which took a per-bank `BankLock` this same round), nothing serializes `load`-then-`store` across processes. `roll_up_spend` (called once per `zirv ctx loop` cycle from `run_loop.rs`) is the sharpest instance: it loads the current record, adds `delta` to `spent_tokens`, and stores the result back -- a classic read-modify-write, with no lock around the pair. Two `zirv ctx loop` processes whose worktrees resolve to the same `repo_slug` (a known collision class, see the 2026-08-21 vault-merge-pass changelog entry above) rolling up spend at nearly the same time can lose one delta entirely -- the later write simply overwrites the earlier one's `spent_tokens` with a value computed before it landed -- silently under-counting the durable objective spend the monotone-spend design exists to protect. Not yet fixed; would need the same per-key lock shape `group.rs`/`memory.rs` already have. See [[Ctx Subsystem]]'s `objective` verb entry.

## Codex rollout transcript pins under `<state>/rollouts/` are never pruned

Recorded 2026-09-06 (`feat/inner-workings-audit-2`, `spend` fix track + review round 1). `codex::forget_transcript_pin` removes a session's `<short>.path` pin file on a handover, but nothing ever removes the `.floor` file it writes alongside it, and nothing removes EITHER file once the session that owns them retires normally (a clean exit, not a handover). Both files are named by the session's own short id, which is not reused, so a long-lived machine accumulates one `.path`/`.floor` pair per codex session ever run, forever. Not a correctness bug (a stale pin for a retired session is simply never read again), but an unbounded-growth gap of the same shape `safety-decisions/` had before this round's retention fix (see the 2026-09-06 [[Decision Log]] entry) -- and this one has no retention mechanism at all yet. See [[Ctx Adapters]]'s "`transcript_path` resolves and pins the rollout codex actually minted" section.

## `setup.rs`'s deterministic restore staging path races two concurrent restores of one target

Recorded 2026-09-06 (`feat/inner-workings-audit-2`, `setup` fix track). `restore_staging_path(dest)` is deliberately deterministic (`<dest's parent>/.{dest's file name}.zirv-restore-tmp`), not unique-per-run, specifically so a staging file left behind by an INTERRUPTED restore is reclaimed by the next one rather than accumulating forever. That same determinism means two restores racing to restore the SAME destination at the same time share one staging path: `restore_target`'s `remove_staged(&staging)` / `copy_for_backup` / final `rename` sequence in one process can observe or clobber the other's in-flight staging file, since nothing locks the staging path itself. `zirv setup restore` has no documented single-instance guarantee, so nothing currently prevents two concurrent invocations. Not yet fixed; would need either a lock file alongside the staging path or a unique-per-run staging name with the interrupted-run cleanup done by a separate sweep instead of by reuse. See `restore_staging_path`'s own doc comment in `src/commands/setup.rs`.

## A single API response with a split thinking block is credited its reported thinking tokens twice

Recorded 2026-09-06 (`feat/inner-workings-audit-2`, `adapters` fix track, "Thinking blocks are sized from the reported thinking tokens" fix). Claude Code writes `AssistantThinking { byte_len }` as one row per thinking-shaped content block, but the actual text is stripped (or the block is `redacted_thinking`) in current builds, so this round sizes such a row instead from that RESPONSE's own `usage.output_tokens_details.thinking_tokens`, scaled to the byte unit `breakdown::attribute_window` uses everywhere else. `parse_events` must stay line-local (each transcript line's events depend only on that line), so there is no way to tell, from one row alone, whether a response's thinking already got credited by an earlier row in the same response -- a response whose thinking content splits across two block rows is credited that response's whole reported thinking-token total on EACH row, double-counting it in `breakdown::attribute_window`'s thinking bucket. Not fixed in this round (fixing it would need cross-row response-identity tracking of the same shape "Usage is folded once per API response" just added for `transcript_usage`, but applied to a different, structural-context-only pass). See [[Ctx Adapters]]'s "Thinking blocks are sized from the reported thinking tokens" section.

## `dash::pane::Pane::launch_mode` still reads the pane's original spawn mode after a handover

Recorded 2026-09-06 (`feat/inner-workings-audit-2`, second full-codebase audit; the underlying gap is a pre-existing 2026-08-28 `NON-GOAL residual` comment in `pane.rs`, now confirmed still live and recorded here). `Pane::handover`'s own `build_turn_env` never pushes the durable interactive-launch pin (`adapters::launch_mode_pin_env`), so `self.launch_mode` -- and everything downstream of it, including `RosterPane::interactive` -- keeps reporting the pane's ORIGINAL spawn mode after a cross-adapter or cross-harness handover swaps its child, even though the successor child never actually receives the pin either way. Out of scope for the fix round that introduced `launch_mode` (issue #160), which named exactly three call sites (`fulfill_spawn_request`, `run_dashboard`'s first pane, `restored_pane_turn_env`), none of them the handover path. Not fixed here; would need `Pane::handover` to recompute and store `launch_mode` the same way `Pane::spawn` does. See `src/commands/ctx/dash/pane.rs`'s own comment at the end of `handover`.

## Step `Options`' new `deny_unknown_fields` is a breaking change for a script with a stray option key

Recorded 2026-09-06 (`feat/inner-workings-audit-2`, `runner` fix track, commit `8d0eb3a`). `Options` now rejects any key outside its closed set (`os`/`interactive`/etc.) with `#[serde(deny_unknown_fields)]`, closing a real gap where a typo'd or stale option key was silently ignored rather than caught -- but a `.zirv/commands/*.yaml` script written against an older zirv build, carrying a since-removed or misspelled option key under a step's `options:` block, now fails to parse where it previously ran (silently ignoring the stray key). No migration path or deprecation window; this is a deliberate, unannounced breaking change to the script schema -- `Options`' own doc comment calls it "a closed key set with no meaning outside the fields below" and explicitly does NOT apply the same treatment to `Command`/`AgentCommand`/`Script`, which existing user scripts may still carry extra keys on -- but it is worth knowing before assuming an existing script fleet parses unchanged after upgrading. See `src/script_runner/options.rs`.

## `--yes` is accepted and silently ignored on every non-`init` command

Recorded 2026-09-06 (`feat/inner-workings-audit-2`, `runner` fix track, commit `e7c45a0`). `Input::yes` lives on the same shared clap `Input` struct as `--name`/`--shortcut`/`--global`, but unlike those three it is NOT included in `Input::misplaced_create_flag`'s check, so nothing warns when `--yes` is passed to an ordinary script invocation (`zirv myscript --yes`) or to `create`/`help`/`version` -- it simply parses and does nothing. `--name`/`--shortcut`/`--global` at least get a "this flag is create-only" hint in the same situation. Not fixed here (adding `--yes` to `misplaced_create_flag` would need its own "init only" wording, since it is not a create flag); the gap is silence, not incorrect behavior, since a script whose own params happen to be named `--yes` would collide with a real clap flag regardless. See `src/input.rs`.

## `prune_safety_buckets` scans its whole directory on every `append_safety` call

Recorded 2026-09-06 (`feat/inner-workings-audit-2`, `state` fix track, commit `2a71fcb`, see the matching 2026-09-06 [[Decision Log]] entry). Enforcing the new 30-day retention on `<state>/logs/safety-decisions/` runs `std::fs::read_dir` over the whole bucket directory inside `append_safety` itself, so every `PreToolUse`/`safety check` hook invocation now pays for a directory scan proportional to how many day-buckets exist (bounded by the retention window, so it cannot grow unbounded, but it is real per-call cost on a hot path that fires on every classified command). Not fixed here -- a cheap alternative (prune only every Nth append, or on a separate timer) was left for a future pass rather than adding complexity to the fix that closed the far more serious unbounded-growth gap. See `src/commands/ctx/log.rs`'s `append_safety`/`prune_safety_buckets`.
...
## A dashboard pane cannot enforce `--max-tool-calls`

Recorded 2026-09-06 (`feat/inner-workings-audit`, T9 fix track, `zirv ctx agent --headless` removal). Since `--headless` no longer exists as an opt-out, every delegation ceiling the operator types now has to travel on the `SpawnRequest` and either be enforced by the pane or honestly reported as unenforceable. `--max-restarts` and `--timeout-secs` both got real pane-side mechanisms (a pane's child is never restarted by zirv at all, so the restart budget is satisfied by construction; `dash::enforce_pane_deadlines` stops a pane whose wall clock runs out exactly the way `exec::run_with`'s own timeout does). `--max-tool-calls` did not: a dashboard pane has no verified tool-call counter the way `exec::run_with`'s own supervised loop does, so `fulfill_spawn_request` pushes a one-line notice naming the gap (`pane_ceiling_notices`) instead of enforcing it, and the pane's token budget is the only ceiling that actually applies. Not a regression from `--headless`'s removal — an inline (`exec`-driven) worker always had `--max-tool-calls` enforced, and before this release a pane-bound delegation carrying that flag hard-errored rather than silently under-enforcing it, so this is a new, honestly-reported gap rather than a newly-introduced silent one. Closing it for real would need a verified per-pane tool-call counter, which nothing in `dash::pane::Pane` tracks today. See [[Ctx Supervisors]]'s "Supervision ceilings on a spawn request" section.
...
## Issue #354's five dashboard phases are all in; these are the residuals they deliberately leave

Recorded 2026-09-05 (`feat/354-dash-ux-audit`, issue #354 phases 1-4). Pointer events used to reach only the focused pane's grid: a click on the sidebar was dropped on the floor, so the roster was keyboard-only while looking exactly like a list you could click. That is closed — `dash::hit::hit_test` plus `dash::route_mouse` give rows, group headers, the summary line and the header hints real pointer targets, and the sidebar wheel scrolls the roster. **Phase 2 closed the attention gaps**: the glyph column, the summary/group rollups and the `reason`/`since` disclosure lines are projections of the cached `attention::SessionStatus` (`ui::glyph_for`, `DiskFacts::attention`, loaded on the `FactsCache` cadence, never per frame), `◆` clears only after an unoccluded render of the focused pane rather than on any focus change, completed workers keep a row (capped at 8), and `^A Left`/`^A Right` share one fold reducer with the header click. **Phase 3 closed the dialog and action gaps**: every list dialog now scrolls through one shared viewport with a pinned title and hint row and a `3–12 of 40` indicator, an open dialog's own rows and hints are clickable while its backdrop stays inert, `^A c`/right-click opens a real context menu whose unavailable entries are drawn with their reason, `^A i` opens a real per-session inspector (evidence and skipped-authority reasons included), `^A r` relaunches an ended row from the spawn request the dashboard kept for it, and a retained ended row can be dismissed by hand. **Phase 4 closed the discoverability gaps**: one action-descriptor table (`dash::actions::ACTIONS`) now feeds the help screen, the header cluster, the context menu and a new `^A p` palette — nothing is written down twice, and a rendering test proves no surface draws a chord the table does not define; help and the palette are one overlay (`Overlay::Palette`, `PaletteMode::{Run,Help}`) with a pinned query row and a case-insensitive fuzzy subsequence filter; `Esc` closes and `Enter` confirms in *every* dialog now (the errors overlay and the inspector had an inert `Enter`, help closed on any key at all); and a first launch shows one dim `^A ? help · ^A p palette · click a row to focus` notice, flagged at `<state>/dash/tip-seen`. **Phase 5 closed the notification gaps**: a pure, clock-injected `dash::notify::NoticeReducer` emits one compact `▲/✗/◆ <short> <words>` notice per transition INTO needs-action/failed/done-unread — suppressed for the focused pane, deduped per `(session, episode)`, delivered through the existing header-middle channel and no new surface; `push_error`'s buffer became `ErrorLog`, collapsing identical consecutive messages into one `×n` entry and clearing the sticky `⚠` line on acknowledgement (`Esc`/`Enter`/`q`/`a` on `^A e`) without ever deleting anything; and `^A i` on the sidebar's summary line opens a dashboard-level inspector (harness/seat, glyph counts, spend and pool headroom, usage windows, mail, workflow, supervision, state dir, uptime, mouse mode, sidebar width) from cached facts only. See [[Ctx Supervisors]]'s "The 44-column sidebar…", "Attention glyphs, done-unread acknowledgement and retained ended rows", "Scrollable list dialogs, the context menu, the inspector and real Restore", "One action table, the palette, consistent Esc/Enter and the first-run tip" and "Attention notifications, error acknowledgement and the dashboard inspector" sections. What is deliberately **not** in, after all five phases:

- **A session already blocked when the dashboard starts never raises a notice.** The reducer only announces a transition it actually observed, so the first sample for a session is silence by design; its `▲` glyph is the only signal until it leaves and re-enters the state. Same for a transition that happens entirely between two `FactsCache` refreshes and back again.
- **A notice suppressed because its pane was focused is never replayed.** Moving focus away afterwards does not re-raise it — the projection has not changed, and re-announcing on every focus move would be noise.
- **`restore`/`retry` are unavailable for a pane the dashboard did not spawn from a request** — the orchestrator itself, and a startup-restored pane. Both kept no `spawnreq::SpawnRequest`, so there is nothing to replay verbatim; the menu says `no spawn request kept` and the `^A r` hint is not drawn for that row. Reconstructing a request from the roster entry (the way `spawn_restored_pane` does) would be a second, differently-gated relaunch path, which [[Command Safety]] rules out.
- **`open worktree` shows the path; it does not open anything.** Spawning a shell or an editor would be a new process-launch path outside the spawn machinery, so the entry puts the cwd in the header notice and opens the inspector on the row instead.
- **`branch` in the disclosure and in the inspector is always the placeholder.** Filling it needs a *cached* branch; the render path must never shell out to git, and no cheap cached value exists yet.
...
Recorded 2026-09-04 (`feat/elastic-scheduling-358`, issue #358 T5). `wrap::perform_handover_swap` (both the manual `zirv ctx handover` path and an automatic rollover's own execution) mirrors an ordinary rot-triggered restart: `quit_child` the old process, open a fresh pty, relaunch -- so by the time `pump`'s tick loop starts polling `rollover::successor_readiness`, the predecessor is already gone. A `TimedOut` successor cannot be rolled back onto the source, because there is no source left to restore; the transaction is aborted and the seat re-registers onto whatever is actually running. `dash::pane::Pane::handover` does not share this gap -- its own ordering runs every fallible step for the successor FIRST, before the old child is touched at all, so a dashboard rollover can still fail cleanly with the predecessor pane untouched. Not yet fixed for `wrap`; would need `perform_handover_swap` to hold the source process open until the successor's readiness (or timeout) is known, the same ordering `Pane::handover` already gets. See [[Ctx Supervisors]]'s "The logical seat and automatic meta-orchestrator rollover" section.

## A reservation left on a provider a mid-run reroute already moved away from is only reclaimed by the dead-owner sweep

Recorded 2026-09-04 (`feat/elastic-scheduling-358`, issue #358 T3/T5). When `exec.rs`'s blocked-session reroute (`route_blocked_session`) moves a running session onto a different harness/provider, it releases the OLD provider's reservation and takes a new one against the new provider (`reservation::release` then `reservation::reserve`, both best-effort). `release` failing (an I/O error against that provider's ledger file) leaves the stale entry behind under the OLD provider, still owned by this same live process -- so it is not double-counted as capacity elsewhere, but it also is not reclaimed until this process exits and the NEXT locked write to that provider's ledger runs `prune_dead` (`reservation.rs`'s own deferred-sweep-on-write design, not a bug specific to the reroute path). A long-lived orchestrator seat that reroutes several times without ever writing to the abandoned provider's ledger again could, in principle, leave more than one such stale entry outstanding at once. Not yet fixed; would need either a proactive sweep at reroute time or a release retry. See [[Usage and Pacing]]'s "Elastic cross-harness capacity" section.

## `HARNESS_PROMPT`'s repository-write sentence stays posture-independent by design, not automatically updated

Recorded 2026-09-04 (`feat/elastic-scheduling-358`, issue #358 T8). Each adapter's OWN `ORCHESTRATOR_PROMPT` (`claude::orchestrator_prompt_for`/`codex::orchestrator_prompt_for`) splices a posture-dependent write-guard bullet via `prompt::orchestrator_write_lines(posture)`, so an orchestrator's own system prompt correctly describes `allow`/`advise`/`deny`. The separate derived harness/orchestration roster layer, `prompt::HARNESS_PROMPT`, does NOT: its own "this seat coordinates and integrates, dispatch a worker for repository changes" bullet is deliberately left unspliced regardless of posture (`prompt.rs`'s own doc comment: the sentence is forward-looking delegation guidance, true under every posture, and splicing it would also force `compile.rs`'s several `HARNESS_PROMPT.len()`-based layer-range computations to become posture-aware for no behavioral gain). This is an intentional design decision, not an oversight -- recorded here so a future reader auditing prompt text for posture-consistency does not "fix" the one sentence that was deliberately left alone. See `prompt.rs`'s `orchestrator_write_lines` doc comment for the full reasoning.

## The chocolatey-installed zirv binary on this machine still carries issue #346

As of this batch (2026-09-04), the chocolatey-installed `zirv.exe` on this Windows machine is 3.18.0, which carries issue #346 (native subagents are denied repo writes) -- until a fixed release is installed system-wide, any harness batch worked on this machine needs a fixed binary reachable ahead of the chocolatey install on `PATH`. This batch built 3.19.0 from the unmerged PR #347 branch (`release/3.17.0-harness-batch-5`'s own successor) and placed it in `~/bin`, which Git Bash resolves before the chocolatey install path, so hooks invoked as plain `zirv` resolve the fixed binary without an elevated reinstall. Remember to remove or update that `~/bin` shim once a real release supersedes both 3.18.0 and 3.19.0, or a future session may silently keep running against a binary two releases behind `main`. Separately (not a zirv defect): codex had no usage quota left for this batch, so every track ran on Claude sonnet instead of the usual cross-harness split.

## `Attention::Question`/`Attention::Permission` have no live source; no `ScreenManifest` authority; no `ctx status`/`explain-status --json`

Issue #349's composed attention model defines nine `Attention` variants and six `Authority` levels, but this batch only wired sources for `Quota`/`WorkflowGate`/`WriterConflict`/`VerificationFailure`/`Stalled` plus the general lifecycle transitions -- `Attention::Question` and `Attention::Permission` exist as enum variants with no caller anywhere that ever constructs them, so they are permanently unreachable until a hook or supervisor is taught to record one. Similarly, `Authority::ScreenManifest` (meant for a captured terminal-screen reading of a genuinely unattended pane, ranked between `Transcript` and `QuietHeuristic` in `AUTHORITY_ORDER`) has no producer at all today -- every live pane's own attention currently resolves through `QuietHeuristic` instead, one rung lower than intended. `zirv ctx status`/`zirv ctx explain-status` both print human-readable text only; neither has a `--json` flag, so a caller that wants the composed `SessionStatus` programmatically has to parse prose. None of these three gaps is a defect in what shipped -- they are the batch's own stated residuals -- but a future change wiring a new hook/supervisor source, or a caller needing machine-readable status, should check here first rather than assume the model is already fully wired.

## Task-card silent-zero-exit detection needs a declared `--result-schema`; no per-tick heartbeat from the supervisor loop

Issue #317's `task.rs` models a worker that exits 0 with no useful output as `ExitKind::SilentZero`, but `agent.rs`'s own completion path (`task_exit_kind`) can only ever produce that verdict when the delegation also declared `--result-schema` (issue #318) -- without one, a plain exit 0 is treated as `ExitKind::Reported` (success), since a bare exit code carries no evidence either way. A `--task <id>` delegation with no `--result-schema` therefore gets none of the "ran, exited clean, but never actually finished the work" protection the recovery policy (`respawn_decision`) is built to catch; an operator who wants that protection for a task card must declare a schema explicitly. Separately, `exec`/`run_loop`'s own tick never calls `task::heartbeat` on a claimed card's behalf -- a claim's TTL only advances via an explicit `zirv ctx task heartbeat`, so a long-running supervised worker holding a task claim depends on either the worker itself calling `heartbeat` or a caller-side wrapper doing it, not on the supervisor loop that is already ticking regardless.
...
## An operator's own `--force` never reaches a dashboard pane (R1-7, 2026-09-06 review round 1)

`zirv ctx agent --force` (spend anyway at the pace ceiling, and disable automatic cross-harness rerouting) reaches `SpawnRequest.force`, but `dash::sanitize_file_dropped_request` clears it unconditionally on every file-backed drop: the drop channel cannot tell an operator's own terminal from a pane's own harness child writing into the same directory (`dash::intake_channels`, the shared `requests` leaf proves nothing, issue #179, accepted residual). An operator who types `--force` and lands on a pane — which happens whenever any dashboard is live anywhere on the machine, not only one this process was launched inside — therefore gets a pane whose spend gate and cross-harness routing were re-decided without it, a demotion the operator previously could not see. `pane_ceiling_notices` now names it on stderr (`"--force is not carried to a dashboard pane: ..."`), the same treatment an unsupported `--max-tool-calls` or trailing flag beyond a model pin already got — but the underlying limitation is unchanged, not fixed: `--force` only ever has effect on the inline fork. See [[Ctx Subsystem]]'s `agent` verb entry and [[Untrusted Configuration]]'s file-drop trust-boundary section.
