...
Blocking detector rules are kept to structural accessibility hazards with relatively objective source evidence; contextual craft/UX rules are advisory because a flagged technique can be legitimate in an established product system. The AI visual review must inspect rendered evidence and use the autonomous profile/product provenance to resolve that context. A pass now requires 13 explicit UI/UX scores at 4/5 or better, no unresolved finding, fresh capture linkage, and the active reviewer/model identity; this makes weak claims more visible and auditable, but a determined or careless agent can still enter inflated scores, and Zirv cannot prove it attended to every pixel or exercised an undiscoverable state. The 27-case benchmark proves all 44 deterministic rules are represented and catches detector drift, not subjective design-quality regressions across model releases.
