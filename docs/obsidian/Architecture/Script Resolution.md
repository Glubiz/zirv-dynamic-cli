...
- Resolution order: **1)** `ctx` interception on raw argv, **2)** `chat`/`agent` top-level alias interception on raw argv, **3)** bare invocation (`argv.len() == 1`) — chat or help, **4)** top-level `--help`/`-h` interception on raw argv, **5)** clap-parsed built-ins (`help`, `version`, `init`, `create` and their short aliases), **6)** literal file path, **7)** local `.zirv/` (direct extension match, then `.shortcuts.yaml`), **8)** global `~/.zirv/` (same two steps).
