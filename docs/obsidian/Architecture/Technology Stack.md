...
| `rusqlite` (`bundled`) | 0.40 | v3.31.0, issue #382 — `ctx::transcript_source::ShadowTranscript::sync_sqlite` reads a harness's own SQLite transcript database read-only (plain `SQLITE_OPEN_READ_ONLY`, `?immutable=1` only as a fallback) and materializes new rows into a zirv-owned shadow JSONL. `bundled` compiles SQLite from source rather than depending on a system library, so the binary stays self-contained the way `ureq`'s vendored TLS story already is. This crate's first SQL dependency, and — like `transcript_source` itself — dormant until a wave-1 adapter (issues #384-#386) calls into it |
