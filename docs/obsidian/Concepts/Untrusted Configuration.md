...
- **Capped.** `remember` truncates an oversized body to `cfg.memory.max_entry_bytes` (default 512) via `crate::utils::truncate_bytes` — the same helper the repo prompt layer and mail both use — and prunes the bank to the newest `cfg.memory.max_entries` (default 50) by the entry's own `Written` timestamp. `render_for_prompt` (the one seam into the injected prompt) separately caps the whole rendered batch to `cfg.memory.max_injected_bytes` (default 2048), the same "cap the batch, not just the entry" shape `mail.max_delivered_bytes` uses. That cap *selects* newest-first (ranked by `Verified`, then `Written`) and renders what fits, rather than rendering the whole bank in storage order and byte-truncating the tail — which delivered only the oldest facts and silently dropped every recent one. The truncation note names how many older entries were omitted, so an over-budget bank is visible rather than merely implied. Every one of these six keys (including `shared_enabled`, the independent gate for the newer repo-owned shared scope — see [[Ctx Subsystem]]'s `MemoryScope`) is repo-forbidden, for the same reason `mail.max_delivered_bytes` is: without the cap being off-limits to the checkout, it would be decorative.
