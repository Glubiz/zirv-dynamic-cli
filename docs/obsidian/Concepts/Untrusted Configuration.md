...
`zirv ctx send`/`zirv ctx inbox` (`src/commands/ctx/mail.rs`, see [[Ctx Subsystem]]) is a fourth text surface, but not a repo-checkout one: a mail message is written by another *agent session* running on the same machine, addressed to this repo. It is not committed config and it is not read-only analysis input — it is delivered into a headless **Worker** session's composed prompt as its own layer (`PromptSource::Mail` in `prompt.rs`, folded in after the repo layer and before the command-line layer — see [[Ctx Supervisors]]), at both places a worker launches (`zirv ctx exec`, including a delegated `zirv ctx agent` run, once at launch; each `zirv ctx loop` cycle, fresh), and only when `cfg.mail.enabled` is true. An interactive **Orchestrator** session (`zirv ctx chat`, via `wrap`) never receives message bodies in its own prompt at all — only a one-line unread-count advisory on the announcement channel, since the whole point of the orchestrator seat is a human deciding what's worth reading.

The same two habits apply anyway, adapted to what mail actually is:

- **Capped.** `store` truncates a message body to `cfg.mail.max_message_bytes` (default 4096) before writing it via `crate::utils::truncate_bytes`, the same helper the repo prompt layer uses, and appends `[truncated]` rather than silently dropping the tail. `with_mail_layer` separately caps the *whole delivered batch* to `cfg.mail.max_delivered_bytes` (also `truncate_bytes`, also `REPO_FORBIDDEN` — see the table above), since several small messages could still add up to more than an operator wants injected at once. The mail directory itself is pruned to the newest `cfg.mail.keep` unread messages, so an unbounded stream of notes can't grow the store forever.
