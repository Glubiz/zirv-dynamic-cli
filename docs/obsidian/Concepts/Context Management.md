...
- **Mailbox.** `zirv ctx send`/`zirv ctx inbox` let agent sessions on the same machine leave each other short markdown notes, scoped to the current repo. Unread mail is folded into an orchestrator session's composed prompt as its own layer (after the repo layer, before the command-line layer), so a chat session starts already aware of what other sessions left it — labeled as information from another session, not as an instruction, the same trust posture given to any other agent-authored text (see [[Untrusted Configuration]]).
