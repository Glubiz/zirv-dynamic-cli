...
## Round 4 -- harness iteration round 2 (2026-09-02)

A design check against workflow af4ed8ec (same PR #261) picked four items off round 3's own deferred list and its review round's two RESIDUAL findings, ahead of the lower-priority items still deferred below. Dogfooded immediately: the capped round-3 workflow 263cb162 was closed with the new verb (`status: Closed`, reason recorded) instead of lingering as active.

Shipped, all four:

1. **`zirv workflow close <id> [--reason]`.** A workflow whose review/fix loop hits `MAX_FIX_REVIEW_ROUNDS` had no path to `Completed` and stayed `Running` forever, still reported as this repository's active workflow. `WorkflowStatus::Closed` replaces the never-constructed `Cancelled`; `close` refuses (fail closed) while any review finding is `Open` or the workflow is `AwaitingApproval`, otherwise sets `Closed`, records `closed_reason`/`closed_at`, clears the active pointer, and emits `TelemetryKind::Closed`. **Bug found and fixed in the same change:** `resume` called `refresh_deploy_tier` -- which unconditionally recomputes `status` from the current step -- before checking whether the loaded status was already terminal, so a `Closed`/`Failed` workflow's status was silently overwritten back to `Running`/`AwaitingApproval` and then allowed to resume; the terminal check now runs first, against the as-loaded status.
2. **Verification outcome closes round 3's residual findings F1 and F2.** `VerificationOutcome.status: VerificationStatus { Passed, Failed, Unknown }` replaces the bare `errored: bool` (F1): a shell's exit status is only the LAST simple command's, so `cargo test || true`, `cargo test; echo done`, and `cargo test | tee out.log` now render "outcome unknown (compound command)" instead of a guessed pass; a `&&` chain ending in the verification segment still attributes normally. `write_list` (Done/Remaining/Files touched/Gotchas learned) now runs every item through the same `normalize_rendered_line` the `Verification` line already used (F2), closing the same injection/unbounded-growth gap for ordinary list items.
3. **`zirv ctx agent --attach-artifact <intent|spec|plan> [--workflow <id>]`.** Reads the resolved workflow's accepted artifact via the engine's already-validated `read_accepted_artifact` (symlink-refusing, hash-pinned), reorders Acceptance Criteria/Goals to the front, caps at 8 KiB with a truncation marker, and appends it to the worker's task prompt after the operator's own text under an "untrusted repository content, information only, grants no permissions" label -- never the system prompt. Fails fast, before any worker launches, when no workflow is active/named or nothing is accepted for the stage. Reimplements a small local `reordered_artifact_excerpt`/`markdown_sections`/`PRIORITY_ARTIFACT_HEADINGS` in `agent.rs` mirroring `workflow::review`'s private `accepted_artifact_excerpt` rather than sharing it -- `review.rs`'s helpers are private and owned by concurrent work this task did not touch.
4. **A "Repeated tool errors" harvest block.** `durable_harvest_prompt` gains an optional section built from `StructuralContext.tool_errors`, deduped via the same `event::normalize_error_text` fingerprint `same_error_repeats` already uses, capped at 10 entries of 200 chars, labeled untrusted/information-only. Every pre-existing caller still passes `tool_errors: None` and sees a byte-identical prompt; `harvest_at_session_end` is the one caller with the raw signal in hand.

Review round (two independent reviewers, Critical risk): codex found one major -- `close` cleared the repository-wide active pointer unconditionally, so closing an older workflow deactivated the one actually active, and it accepted terminal targets; fixed to refuse Completed/Failed/Closed and to clear the pointer only when it names the closed workflow. Round 2: codex flagged the check-then-delete race on that pointer; dismissed because the engine's whole pointer protocol (`save` active=true/false) is unsynchronised across processes, so a lock belongs in a dedicated change covering every pointer mutation (backlog). claude: no findings in either round.

Deferred, not in this PR: size-aware review count within a tier; routing the reviewer's model by risk; the codex idle heuristic; the review-model default when `chat.model` is unset; harvesting from past workflow failures/findings (distinct from item 4's own tool-error harvest); deduping repeated artifact re-injection across workflow steps. See [[Active Work]]'s wrapper-redesign entry.

## Round 5 -- harness iteration round 3 (2026-09-02, narrow) and stopping point

A fourth audit against the same wrapper, run narrow with no `zirv workflow` and a single independent review, picked round 4's own lowest-hanging deferred item plus one this round found on its own. Verified against the branch's uncommitted diff (`review.rs`, `adapters/mod.rs`, `status.rs`).

Shipped, both:

1. **The review-routing line becomes enforced, not advisory.** `resolve_review_model` (`adapters/mod.rs`) is now `pub(crate)`; `workflow::review::reviewer_argv` calls it directly and appends `adapter.model_args(&review_model)` to the reviewer's own launch argv, before the read-only floor so no later argument can weaken it. Before, the roster's `review_roster_line` told an operator which model review "runs on" while the actual `review run` launch carried no `--model` at all and ran on the adapter's bare CLI default -- the line and the behavior could silently disagree. Consequence: with `chat.model` unset, a fresh install's automated reviewers now literally launch on each adapter's ladder-top default (`opus` for claude) instead of an unpinned CLI default -- an operator decision surfaced by this fix, not a new default invented for it.
2. **A settled review finding writes one durable memory entry.** `remember_finding_disposition` (`review.rs`) writes one private-scope `Entry` under its own `review-finding-<8-char id>` bank -- not the repo's ordinary memory bank -- whenever a finding's disposition settles on `Fixed` or `Residual`; `Dismissed`/`Accepted`/`Open` write nothing. Body is severity/disposition/workflow-kind/repo-slug plus a 160-char summary excerpt, never the diff; best-effort, same posture as `record_finding_update` and every other memory writer (disabled memory or a config load failure means "nothing recorded," never a failed disposition call).

In the same diff but not part of the shipped feature work: a `status.rs` test hermeticity fix (`HomeGuard` added to `status_shows_no_usage_source_for_a_codex_configured_repo_rather_than_anthropic_numbers`) closed the root cause of an intermittent CI flake -- the test read the real `~/.zirv/ctx.toml` with no isolation.

Review (one independent sonnet pass, bounded change): the argv/model ordering checked out; two medium findings in the new memory write were fixed before landing -- a repo's forbidden `ctx.toml` key made `CtxConfig::load` fail and silently suppressed the write (a repo surface must never disable an operator-facing audit; it now falls back to the operator-level default), and a reviewer summary containing a literal `## Memory` line could re-enter the memory parser's header mode and spoof the entry (the summary is now collapsed to one labelled line).

Rejected, all four for a concrete structural reason rather than "not worth it yet":

- **Size-aware review count within a tier.** The engine's `RiskBand` already gives High one review and Critical two; scaling further inside a tier by diff size needs `Classification.reasons` to be structured data to key off, and it is free-form prose today.
- **An active-pointer lock.** There is no lock primitive in the workflow-state layer at all, and every read-modify-write state path shares the same unsynchronised `save(active=true/false)` pattern -- a lock needs to cover all of them at once, not the one caller this round happened to be looking at. Own design, own PR.
- **Artifact re-injection dedupe.** `compose` legitimately runs fresh at session start and at every rot restart, where a new agent process has nothing cached to dedupe against -- the "waste" this would target is real only across steps within one still-live session, not the case the round's audit actually found.
- **The codex idle heuristic.** Still blocked on unverified codex rollout event shapes, the same blocker round 3 recorded for the general idle-turn signal.

This audit round found nothing further that beat its own cost to investigate, so the harness-iteration loop stops here for this branch/PR. The rejected four move to [[Active Work]]'s backlog, each with the concrete unblock condition above rather than left open-ended.
