...
- **Codex:** token from `~/.codex/auth.json` (`tokens.access_token`, falling back to a top-level `access_token`) against the ChatGPT-backend usage endpoint. **This path ships unverified** (probed 2026-08-16: no readable token on the reference machine, so the endpoint could not be exercised): the code ships best-effort, every failure degrades silently per the table below, live-endpoint tests are impossible, and Known Issues records it as unverified. Codex's *passive* collector is the verified, primary source.
