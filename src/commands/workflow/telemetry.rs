...
pub fn record(
    ...
    write_private(&path, &serde_json::to_string(event)?)?;
...
pub fn list(state: &StateDir, repo: &Path) -> CtxResult<Vec<TelemetryEvent>> {
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        if let Ok(event) = serde_json::from_str(&std::fs::read_to_string(entry.path())?) {
            events.push(event);
        }
    }
...
pub fn aggregate(events: &[TelemetryEvent]) -> StatsReport {
    ...
    let mut findings_meaningful = 0u64;
    let mut findings_dismissed = 0u64;
    for event in events {
        ...
        findings_total += u64::from(event.findings_total);
        findings_meaningful += u64::from(event.findings_meaningful);
        findings_dismissed += u64::from(event.findings_dismissed);
    }
