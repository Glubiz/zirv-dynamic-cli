...
Inspect the built result, not the implementation story. Review all captures together against the contract, the interchangeable-product test, the full quality rubric, and the primary user path. Batch concrete fixes, then confirm once; open-ended polish loops are forbidden. A clean detector is only a floor, and missing or stale render evidence can never support a visual-quality claim."#,
        ...
        manifest(
            ...
            "Inspect the final diff and require fresh detector, project test, behavioral, accessibility, performance, render, and scored AI-review evidence for every affected surface. Run `zirv frontend check`, `zirv frontend render`, inspect every narrow/intermediate/wide capture, and record the complete review rubric autonomously when evidence is missing; never hand setup or judgment to a human. Confirm all evidence matches the final change/profile fingerprints and the primary journey, including material loading, empty, partial, error, success, disabled, permission, recovery, focus, zoom, localization, long-content, and reduced-motion states. Require every review dimension ≥4, no blocking detector issue, and no unresolved visual finding. Reapply the brief, surface mode, thesis/signature, established system, and interchangeable-product test. State exact passed, failed, unavailable, and skipped evidence. Missing tools, stale captures, or source inspection alone are not visual-quality proof.",
        ),
        manifest(
            "plan",
            "Plan",
