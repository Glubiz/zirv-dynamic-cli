...
```bash
curl -sSfL https://raw.githubusercontent.com/Glubiz/zirv-dynamic-cli/main/install.sh | sh -s -- 2.38.1
```
...
The prebuilt Linux release is x86_64-only. On other architectures (aarch64, armv7, ...), both the install script and the Homebrew formula fail fast — build from source instead (see below).
