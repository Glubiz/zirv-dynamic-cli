...
## Build and verify (all five before done)
...
Nextest's process-per-test isolation prevents `env::set_var` races. `--no-fail-fast` is mandatory: diff complete sorted failure-NAME lists, never counts. The serial `cargo test` fallback must pass too.
...
- Report zirv bugs/gaps: `zirv report bug|feature <title> [--body <text>|--body-file <path>]`; never include secrets.
